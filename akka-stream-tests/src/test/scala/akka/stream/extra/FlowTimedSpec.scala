...
class FlowTimedSpec extends AkkaSpec with ScriptedTest {
  ...
  "Timed Flow" must {
    ...
    "measure time it between elements matching a predicate" in StreamTestKit.checkThatAllStagesAreStopped {
      val probe = TestProbe()
    ...
    "measure time from start to complete, by wrapping operations" in StreamTestKit.checkThatAllStagesAreStopped {
      val probe = TestProbe()
