...
class FlowConcatAllSpec extends AkkaSpec {
  ...
  "ConcatAll" must {
    ...
    "on onError on master stream cancel the current open substream and signal error" in checkThatAllStagesAreStopped {
    ...
    "on onError on open substream, cancel the master stream and signal error " in checkThatAllStagesAreStopped {
      val publisher = StreamTestKit.PublisherProbe[Source[Int, _]]()
      val subscriber = StreamTestKit.SubscriberProbe[Int]()
      Source(publisher).flatten(FlattenStrategy.concat).to(Sink(subscriber)).run()
    ...
    "on cancellation cancel the current open substream and the master stream" in checkThatAllStagesAreStopped {
      val publisher = StreamTestKit.PublisherProbe[Source[Int, _]]()
      val subscriber = StreamTestKit.SubscriberProbe[Int]()
      Source(publisher).flatten(FlattenStrategy.concat).to(Sink(subscriber)).run()
