...
class FlowOnCompleteSpec extends AkkaSpec with ScriptedTest {
  ...
  "A Flow with onComplete" must {
    ...
    "invoke callback on normal completion" in checkThatAllStagesAreStopped {
    ...
    "invoke callback for an empty stream" in checkThatAllStagesAreStopped {
      val onCompleteProbe = TestProbe()
      val p = StreamTestKit.PublisherProbe[Int]()
      Source(p).to(Sink.onComplete[Int](onCompleteProbe.ref ! _)).run()
      val proc = p.expectSubscription
    ...
    "invoke callback after transform and foreach steps " in checkThatAllStagesAreStopped {
      val onCompleteProbe = TestProbe()
      val p = StreamTestKit.PublisherProbe[Int]()
