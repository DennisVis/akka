...
class FlowSplitWhenSpec extends AkkaSpec {
  ...
  "splitWhen" must {
    ...
    "support cancelling substreams" in checkThatAllStagesAreStopped {
    ...
    "support cancelling the master stream" in checkThatAllStagesAreStopped {
      new SubstreamsSupport(splitWhen = 5, elementCount = 8) {
        val s1 = StreamPuppet(getSubFlow().runWith(Sink.publisher))
    ...
    "fail stream when splitWhen function throws" in checkThatAllStagesAreStopped {
