...
class FlowStageSpec extends AkkaSpec(ConfigFactory.parseString("akka.actor.debug.receive=off\nakka.loglevel=INFO")) {
  ...
  "A Flow with transform operations" must {
    "produce one-to-one transformation as expected" in checkThatAllStagesAreStopped {
    ...
    "produce one-to-several transformation as expected" in checkThatAllStagesAreStopped {
      val p = Source(List(1, 2, 3)).runWith(Sink.publisher)
      val p2 = Source(p).
    ...
    "support emit onUpstreamFinish" in checkThatAllStagesAreStopped {
    ...
    "allow early finish" in checkThatAllStagesAreStopped {
    ...
    "report error when exception is thrown" in checkThatAllStagesAreStopped {
      val p = Source(List(1, 2, 3)).runWith(Sink.publisher)
      val p2 = Source(p).
        transform(() ⇒ new StatefulStage[Int, Int] {
          override def initial = new State {
    ...
    "support emit of final elements when onUpstreamFailure" in checkThatAllStagesAreStopped {
      val p = Source(List(1, 2, 3)).runWith(Sink.publisher)
      val p2 = Source(p).
    ...
    "support cancel as expected" in checkThatAllStagesAreStopped {
      val p = Source(List(1, 2, 3)).runWith(Sink.publisher)
      val p2 = Source(p).
        transform(() ⇒ new StatefulStage[Int, Int] {
          override def initial = new State {
    ...
    "support producing elements from empty inputs" in checkThatAllStagesAreStopped {
