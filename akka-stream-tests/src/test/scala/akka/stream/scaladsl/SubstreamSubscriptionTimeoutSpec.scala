...
class SubstreamSubscriptionTimeoutSpec(conf: String) extends AkkaSpec(conf) {
  ...
  "groupBy" must {
    ...
    "timeout and cancel substream publishers when no-one subscribes to them after some time (time them out)" in checkThatAllStagesAreStopped {
    ...
    "timeout and stop groupBy parent actor if none of the substreams are actually consumed" in checkThatAllStagesAreStopped {
      val publisherProbe = StreamTestKit.PublisherProbe[Int]()
