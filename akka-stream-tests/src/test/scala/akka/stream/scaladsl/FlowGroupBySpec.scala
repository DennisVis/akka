...
class FlowGroupBySpec extends AkkaSpec {
  ...
  "groupBy" must {
    ...
    "accept cancellation of substreams" in checkThatAllStagesAreStopped {
      new SubstreamsSupport(groupCount = 2) {
    ...
    "accept cancellation of master stream when not consumed anything" in checkThatAllStagesAreStopped {
    ...
    "accept cancellation of master stream when substreams are open" in checkThatAllStagesAreStopped {
    ...
    "work with empty input stream" in checkThatAllStagesAreStopped {
    ...
    "abort on onError from upstream" in checkThatAllStagesAreStopped {
      val publisherProbeProbe = StreamTestKit.PublisherProbe[Int]()
      val publisher = Source(publisherProbeProbe).groupBy(_ % 2).runWith(Sink.publisher)
      val subscriber = StreamTestKit.SubscriberProbe[(Int, Source[Int, _])]()
      publisher.subscribe(subscriber)
    ...
    "abort on onError from upstream when substreams are running" in checkThatAllStagesAreStopped {
      val publisherProbeProbe = StreamTestKit.PublisherProbe[Int]()
      val publisher = Source(publisherProbeProbe).groupBy(_ % 2).runWith(Sink.publisher)
      val subscriber = StreamTestKit.SubscriberProbe[(Int, Source[Int, _])]()
      publisher.subscribe(subscriber)
    ...
    "fail stream when groupBy function throws" in checkThatAllStagesAreStopped {
      val publisherProbeProbe = StreamTestKit.PublisherProbe[Int]()
      val exc = TE("test")
    ...
    "pass along early cancellation" in checkThatAllStagesAreStopped {
