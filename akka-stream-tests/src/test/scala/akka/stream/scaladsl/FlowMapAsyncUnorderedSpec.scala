...
class FlowMapAsyncUnorderedSpec extends AkkaSpec {
  ...
  "A Flow with mapAsyncUnordered" must {
    ...
    "produce future elements in the order they are ready" in checkThatAllStagesAreStopped {
    ...
    "signal error from mapAsyncUnordered" in checkThatAllStagesAreStopped {
      val latch = TestLatch(1)
      val c = StreamTestKit.SubscriberProbe[Int]()
      implicit val ec = system.dispatcher
