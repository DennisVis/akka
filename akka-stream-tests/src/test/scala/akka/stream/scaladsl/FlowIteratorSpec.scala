...
abstract class AbstractFlowIteratorSpec extends AkkaSpec {
  ...
  testName must {
    "produce elements" in checkThatAllStagesAreStopped {
    ...
    "complete empty" in checkThatAllStagesAreStopped {
    ...
    "produce elements with multiple subscribers" in checkThatAllStagesAreStopped {
    ...
    "produce elements to later subscriber" in checkThatAllStagesAreStopped {
      val p = createSource(3).runWith(Sink.fanoutPublisher(2, 4))
      val c1 = StreamTestKit.SubscriberProbe[Int]()
      val c2 = StreamTestKit.SubscriberProbe[Int]()
      p.subscribe(c1)
    ...
    "produce elements with one transformation step" in checkThatAllStagesAreStopped {
    ...
    "produce elements with two transformation steps" in checkThatAllStagesAreStopped {
    ...
    "not produce after cancel" in checkThatAllStagesAreStopped {
      val p = createSource(3).runWith(Sink.publisher)
      val c = StreamTestKit.SubscriberProbe[Int]()
      p.subscribe(c)
      val sub = c.expectSubscription()
