...
class FlowBufferSpec extends AkkaSpec {
  ...
  "Buffer" must {
    ...
    "pass elements through a chain of backpressured buffers of different size" in checkThatAllStagesAreStopped {
    ...
    "fail upstream if buffer is full and configured so" in checkThatAllStagesAreStopped {
      val publisher = StreamTestKit.PublisherProbe[Int]
      val subscriber = StreamTestKit.SubscriberProbe[Int]()
