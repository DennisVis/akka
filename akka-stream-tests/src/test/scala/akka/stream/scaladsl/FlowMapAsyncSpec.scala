...
class FlowMapAsyncSpec extends AkkaSpec {
  ...
  "A Flow with mapAsync" must {
    ...
    "produce future elements" in checkThatAllStagesAreStopped {
    ...
    "signal future failure" in checkThatAllStagesAreStopped {
    ...
    "signal error from mapAsync" in checkThatAllStagesAreStopped {
      val latch = TestLatch(1)
      val c = StreamTestKit.SubscriberProbe[Int]()
      implicit val ec = system.dispatcher
    ...
    "resume after future failure" in checkThatAllStagesAreStopped {
      val c = StreamTestKit.SubscriberProbe[Int]()
      implicit val ec = system.dispatcher
      val p = Source(1 to 5)
        .mapAsync(4, n ⇒ Future {
    ...
    "finish after future failure" in checkThatAllStagesAreStopped {
    ...
    "should handle cancel properly" in checkThatAllStagesAreStopped {
