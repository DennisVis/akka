...
abstract class TwoStreamsSetup extends AkkaSpec {
  ...
  def commonTests() = {
    "work with two immediately completed publishers" in checkThatAllStagesAreStopped {
      val subscriber = setup(completedPublisher, completedPublisher)
      subscriber.expectSubscriptionAndComplete()
    }

    "work with two delayed completed publishers" in checkThatAllStagesAreStopped {
      val subscriber = setup(soonToCompletePublisher, soonToCompletePublisher)
      subscriber.expectSubscriptionAndComplete()
    }

    "work with one immediately completed and one delayed completed publisher" in checkThatAllStagesAreStopped {
      val subscriber = setup(completedPublisher, soonToCompletePublisher)
      subscriber.expectSubscriptionAndComplete()
    }

    "work with two immediately failed publishers" in checkThatAllStagesAreStopped {
      val subscriber = setup(failedPublisher, failedPublisher)
      subscriber.expectSubscriptionAndError(TestException)
    }

    "work with two delayed failed publishers" in checkThatAllStagesAreStopped {
    ...
    "work with one immediately failed and one delayed failed publisher (case 1)" in checkThatAllStagesAreStopped {
      val subscriber = setup(soonToFailPublisher, failedPublisher)
      subscriber.expectSubscriptionAndError(TestException)
    }

    "work with one immediately failed and one delayed failed publisher (case 2)" in checkThatAllStagesAreStopped {
